...
from kalman.filter import CycleResult
from kalman.ingestion import ProcessedRecord
...
def ingest_dedupe_key_for_persist(
    ...
    *,
    ...
    Key dạng ``live|{run_id}|{UTC ISO-8601 timestamp}``, tối đa một dòng live
    cho mỗi run và mỗi timestamp sensor, giúp retry không ghi trùng.
    """
    if sample_ts is None:
        raise ValueError("ingest_dedupe_key_for_persist: sample_ts required")
    ts = sample_ts
...
def map_result_to_cycle(
    result: CycleResult,
    run: ExperimentRun,
    slice_type: str = PipelineCycle.SliceType.ONLINE,
    source_type: str = PipelineCycle.SourceType.LIVE,
    ...
    slice_type:
        ``"online"`` trong runtime hiện tại.
    source_type:
        ``"live"`` trong runtime hiện tại.
    record:
    ...
    if run.greenhouse_id is None:
        raise ValueError("map_result_to_cycle requires run.greenhouse_id")
