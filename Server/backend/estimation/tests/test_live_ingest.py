...
* Run guards: run not found → 404; pending run → 409;
  completed run → 409.
...
* preprocess_single unit tests (pure logic).
* validate_live_record pure unit tests.
* _restore_state pure unit tests.
* Authorization: only the greenhouse owner's token may ingest (403 otherwise).
...
from estimation.models import ExperimentConfig, ExperimentRun, Greenhouse, PipelineCycle
from estimation.api.ingest import (
    _build_raw_record,
    _normalize_sample_ts,
    _restore_state,
)
import estimation.api.ingest as ingest_module
from estimation.pipeline.store import ingest_dedupe_key_for_persist
from kalman.filter import KalmanConfig, KalmanState
from kalman.ingestion import RawRecord, ValidationResult, preprocess_single
...
@pytest.fixture
def greenhouse(db, device_user):
    return Greenhouse.objects.create(owner=device_user, name="Device Greenhouse")


@pytest.fixture
def live_run(db, greenhouse):
    """Active live ExperimentRun with default ExperimentConfig and greenhouse."""
...
# -- Object-level authorization (Greenhouse.owner) ---------------------------


@pytest.mark.django_db
def test_other_user_token_returns_403_no_cycle(other_auth_client, live_run, payload):
    """Token for a different user must not ingest into another owner's greenhouse."""
...
@pytest.mark.django_db
def test_inactive_greenhouse_returns_403(auth_client, db, device_user):
    """Inactive greenhouses cannot accept live samples."""
    inactive_greenhouse = Greenhouse.objects.create(
        owner=device_user,
        name="Inactive Greenhouse",
        is_active=False,
    )
    run = ExperimentRun.objects.create(
        name="inactive-live",
        run_type=ExperimentRun.RunType.LIVE,
        status=ExperimentRun.Status.RUNNING,
        greenhouse=inactive_greenhouse,
    )
    ExperimentConfig.objects.create(run=run)
...
def test_stored_cycle_has_live_source_type(auth_client, payload, live_run):
    ...
    assert cycle.greenhouse_id == live_run.greenhouse_id
...
@pytest.mark.django_db
def test_pending_live_run_returns_409(auth_client, db, greenhouse):
    run = ExperimentRun.objects.create(
        name="pending-live",
        run_type=ExperimentRun.RunType.LIVE,
...
@pytest.mark.django_db
def test_completed_live_run_returns_409(auth_client, db, greenhouse):
    run = ExperimentRun.objects.create(
        name="completed-live",
        run_type=ExperimentRun.RunType.LIVE,
...
@pytest.mark.django_db
def test_status_guard_inside_lock_prevents_cycle_creation(auth_client, db, greenhouse):
...
@pytest.mark.django_db
def test_second_live_sample_uses_arx_prior_when_history_available(
    auth_client, payload, live_run, monkeypatch
):
    """Live path must pass a cached ARX adapter and prior history into Kalman."""
    ...
    class FakeAdapter:
        ...
        def predict(self, inp):
            from kalman.prediction import PredictionResult

            assert len(inp.history) == 1
            return PredictionResult(value=44.0, status="ok", model_kind="arx")

    monkeypatch.setattr(
        ingest_module,
        "_load_live_arx_adapter",
        lambda: FakeAdapter(),
    )

    first = auth_client.post(_INGEST_URL, payload, format="json")
    ...
    second = auth_client.post(_INGEST_URL, second_payload, format="json")
    assert second.status_code == 201

    cycle = PipelineCycle.objects.get(run=live_run, cycle_index=1)
    assert cycle.arx_predicted == pytest.approx(44.0)
    assert cycle.kf_x_prior == pytest.approx(44.0)
...
def test_reconnect_after_error_cycle_resets_state(auth_client, payload, live_run):
    ...
    PipelineCycle.objects.create(
        run=live_run,
        greenhouse=live_run.greenhouse,
        sample_ts=ts,
        cycle_index=0,
...
@pytest.mark.django_db
def test_missing_experiment_config_uses_defaults(auth_client, db, greenhouse):
...
def test_validate_live_record_all_present_valid():
    from kalman.ingestion.validator import validate_live_record, DEFAULT_CONFIG
