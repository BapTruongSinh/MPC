...
from estimation.models import ExperimentRun, Greenhouse, PipelineCycle
...
def run(db) -> ExperimentRun:
    user = get_user_model().objects.create_user(username="store-user")
    greenhouse = Greenhouse.objects.create(owner=user, name="Store Greenhouse")
    return ExperimentRun.objects.create(name="live", greenhouse=greenhouse)
...
def test_map_result_defaults_to_live_online(run: ExperimentRun) -> None:
    ...
    assert cycle.greenhouse_id == run.greenhouse_id
