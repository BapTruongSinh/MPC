...
# --- Prediction artifact ---
# Runtime live/online ingestion uses a pre-built ARX artifact. Deployment can
# provide it inside Server/backend/artifacts or override with ARX_MODEL_PATH.
def _default_arx_model_path() -> str:
    """Return a deploy-friendly default while preserving monorepo dev fallback."""
    server_artifact = BASE_DIR / "artifacts" / "arx_model.json"
    monorepo_artifact = REPO_ROOT / "ARX" / "arx_model.json"
    if server_artifact.exists():
        return str(server_artifact)
    if monorepo_artifact.exists():
        return str(monorepo_artifact)
    return str(server_artifact)


ARX_MODEL_PATH = os.environ.get("ARX_MODEL_PATH", _default_arx_model_path())
