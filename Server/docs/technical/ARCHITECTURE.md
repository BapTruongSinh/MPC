...
The current primary runtime is an online greenhouse state-estimation pipeline plus a production AMPC recommendation path. Live ingestion accepts authenticated sensor samples, validates and preprocesses each sample, loads a cached ARX prediction adapter from the pretrained artifact at `../ARX/arx_model.json`, reconstructs the minimal live history for the run, uses the estimator module for an Adaptive Kalman-ready update, then stores raw, predicted, and filtered values. AMPC then reads that persisted Kalman state/history by `greenhouse_id`, loads the server-side ARX plant artifact, applies residual bias correction, solves the single-pump MPC problem, stores an audit recommendation, and optionally dispatches a fail-safe actuator command.
...
The first estimator target is scalar `Soil_Moisture`, the minimal adaptive mechanism is bounded innovation-driven adaptive `R` with fixed-per-run `Q`, and the controller path is single-actuator AMPC for pump seconds. Real hardware actuation remains gated by explicit per-greenhouse actuator config; default behavior is recommendation-only.
...
Authenticated user + greenhouse_id
        |
        v
Verify Greenhouse.owner and active status
        |
        v
Load latest valid PipelineCycle + history by greenhouse_id
        |
        v
Load server-side ARXPlantModel from settings.ARX_MODEL_PATH
        |
        v
BiasCorrectedPlantModel + GridShootingSolver
        |
        +--> AMPCRecommendation audit row
        |
        +--> Optional HTTP actuator command (fail-closed)
```
...
The dashboard is a single-page Vite + React + TypeScript application located at `Server/dashboard/`.
...
**API style**: Django REST Framework resource endpoints under `/api/`.

**Middleware / service stack**:
1. DRF authentication for live ingest and AMPC/control endpoints.
2. Object authorization through `Greenhouse.owner`.
3. Request serializers for ingestion and control-profile writes.
4. Service-layer orchestration for ingestion, persistence, evaluation, and AMPC.
5. Endpoint-specific error envelopes for AMPC/control responses.

**Service layer pattern**: Keep preprocessing, prediction adapter, Adaptive Kalman-ready estimator, storage, evaluation, and AMPC controller logic as separate modules.

AMPC integration package:

```text
estimation/control/
  config.py        # GreenhouseControlProfile -> mpc.ControllerConfig
  state_source.py  # latest valid PipelineCycle and PlantRecord history
  bias.py          # residual bias state from kf_x_posterior - arx_predicted
...
2. Backend verifies greenhouse ownership (`ExperimentRun.greenhouse.owner`) and `run.status == "running"` inside a locked transaction.
...
8. Backend stores timestamp, raw measurement, ARX prediction, filtered estimate, innovation/residual, `P`, `R`, config, status, and greenhouse scope.
9. Dashboard and evaluation APIs read the persisted `PipelineCycle` rows.
```

---

### AMPC Online Recommendation Flow

```text
1. Dashboard/API client calls POST `/api/greenhouses/{greenhouse_id}/ampc/recommendations/`.
2. Backend verifies authentication and `Greenhouse.owner` before reading state.
3. Backend creates/loads the greenhouse control profile and maps it to `mpc.ControllerConfig`.
4. Backend selects the newest valid `PipelineCycle` for the greenhouse and rejects stale/future state.
5. Backend builds causal `mpc.PlantRecord` history from the same greenhouse.
6. Backend loads `ARXPlantModel` from `settings.ARX_MODEL_PATH`; request payloads cannot override this path.
7. Backend builds an adaptive bias from recent residuals and wraps the plant model in `BiasCorrectedPlantModel`.
8. `GridShootingSolver` returns pump seconds, predicted soil-moisture horizon, cost, safety status, and reason.
9. Backend persists `AMPCRecommendation` with config/state snapshots and daily pump usage.
10. If actuator is enabled and config is valid, backend sends a Bearer-auth HTTP command; all failures persist/send pump `0`.
```
...
## Adaptive Kalman Cycle (`kalman.filter`)

The `kalman.filter` package provides the v1 scalar state-estimator for
...
`kalman.evaluation` has **no Django dependency** and can be imported
...
## AMPC Modeling And Runtime Boundary

The current runtime AMPC integration is deliberately narrow: one greenhouse, one pump command (`pump_seconds`) per controller step, ARX plant artifact, measured-hold disturbance forecast, and residual bias correction. Hybrid/hierarchical MPC remains unnecessary for the current one-actuator scope.

The implemented AMPC controller path uses the following contracts and preserves extension points for later physical water-balance work:
...
For the current research synthesis, see [`ADAPTIVE_KALMAN_AMPC_NOTES.md`](./ADAPTIVE_KALMAN_AMPC_NOTES.md). For the **implementation-ready AMPC handoff** (state / control / disturbances / cost / safety / Kalman interface, FR-025–FR-027), see [`AMPC_MODELING_HANDOFF.md`](./AMPC_MODELING_HANDOFF.md). For the production Django online decision, see ADR-006 in [`DECISIONS.md`](./DECISIONS.md).
...
**Authentication model**: DRF token auth for live sensor ingestion. AMPC/control-profile endpoints require authentication in every environment and support token/session auth.

**Authorization**: `Greenhouse.owner` is the single ownership source. AMPC verifies ownership before reading state/history, loading the solver path, persisting recommendations, or calling an actuator.

**Data protection**:
- No secrets or credentials committed to the repository.
- Operational endpoints must not be exposed publicly without authentication.
- API responses do not expose `settings.ARX_MODEL_PATH`, actuator token values, or actuator token env var names.
- Actuator URL validation rejects obvious local/private-network SSRF targets before HTTP client construction.
...
| Full ET0/ETc/Dr water-balance controller | Current AMPC uses soil moisture and ARX artifact directly | Add calibrated physical water-balance state only when sensors/parameters are available |
| Real actuator production rollout | HTTP actuator branch exists but defaults off and requires operator config/env token | Pilot with fake/staging actuator before enabling greenhouse hardware |
