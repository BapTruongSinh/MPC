...
- `Kalman/backend/.env`: local secret/config. Không đọc nội dung vào docs, không commit.
- `Kalman/backend/__pycache__/`, `Kalman/backend/**/__pycache__`: Python bytecode cache.
- `Kalman/backend/.pytest_cache/`: cache của pytest.
- `Kalman/dashboard/node_modules/`: dependency đã install từ npm.
- `Kalman/dashboard/dist/`: frontend build output.
- `Kalman/dashboard/node_modules/.vite/`, `node_modules/.tmp/`: cache của Vite/TypeScript/Vitest.
...
Backend nằm trong `Kalman/backend/`.
...
Dashboard nằm trong `Kalman/dashboard/`.
