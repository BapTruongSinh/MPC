...
- [ ] Candidate state choice is documented, including `theta` soil moisture and/or `Dr` root-zone depletion.
- [ ] Candidate control inputs are documented, including drip/pump seconds, mist seconds, and fan duration or level when available.
- [ ] Candidate disturbances are documented, including ET0/ETc if used, temperature, humidity, and light.
- [ ] Cost terms are documented: zone/range tracking, water/energy penalty, actuator switching penalty, and `Delta u` smoothing.
- [ ] Safety constraints and fallbacks are documented: soil moisture bounds, RH max, daily water cap, no mist at high RH/night, emergency fallback, and sensor-fault fallback.
- [ ] The note states whether v1 implements only AMPC contracts or also a minimal offline optimizer prototype.
