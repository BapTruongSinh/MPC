...
The capacitive sensor state remains a 0-100 percent signal. Runtime control
maps the configured sensor target band onto FAO depletion terms: target_high
means field capacity (Dr = 0), target_low means RAW, and the derived sensor
wilting point means TAW.
"""
...
class Fao56Config:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0.0 <= self.theta_wp < self.theta_fc <= 0.8):
            raise ValueError(
                "theta values must satisfy 0 <= theta_wp < theta_fc <= 0.8"
            )
